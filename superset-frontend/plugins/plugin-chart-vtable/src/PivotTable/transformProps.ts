...
import { PivotTableConstructorOptions } from '@visactor/vtable';
...
export default function transformProps(chartProps: ChartProps): PivotTableTransformedProps {
  ...
  // // 提取行列字段和指标
  // const rowFields = groupbyRows.map(field => ({ 
  //   field, 
  //   title: field 
  // }));
  
  // const columnFields = groupbyColumns.map(field => ({ 
  //   field, 
  //   title: field 
  // }));
  
  // const valueFields = metrics.map(metric => ({
  //   field: metric,
  //   title: metric,
  //   aggregation: aggregateFunction.toLowerCase()
  // }));
  ...
  //   sort: {
  //     rowSortState: {
  //       order: rowOrder === 'key_a_to_z' ? 'asc' : 'desc'
  //     },
  //     columnSortState: {
  //       order: colOrder === 'key_a_to_z' ? 'asc' : 'desc'
  //     }
  //   }
  // };
  const data = [
